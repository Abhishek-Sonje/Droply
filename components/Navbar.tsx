...
function NavbarPage({ activeTab }: { activeTab: string }) {
  ...
  return (
    <Navbar
      position="static"
      maxWidth="full"
      className="w-full justify-items-stretch bg-[#077A7D]  rounded-3xl "
      classNames={{
        item: [
          "flex",
          "relative",
          ...
          "data-[active=true]:after:bg-[#7AE2CF]",
        ],
      }}
    >
      <NavbarBrand className="cursor-pointer">
        ...
        <Link href="/" className="text-xl font-bold">
          <CloudUpload className="text-[#ffffff] size-9" />
        </Link>
      </NavbarBrand>
      ...
      <NavbarContent as="div" justify="end">
        {isSignedIn ? (
          <Dropdown placement="bottom-end" >
            <DropdownTrigger>
              <Button className="font-semibold bg-[#06202B] text-[#ffffff]">{(user && user.firstName) || "User"}</Button>
            </DropdownTrigger>
